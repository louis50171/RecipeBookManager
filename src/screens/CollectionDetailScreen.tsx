  ...
  if (!collection) {
    return (
      <View style={styles(theme).container}>
        <Text style={styles(theme).emptyText}>Collection non trouvée</Text>
      </View>
    );
  }
  ...
  const renderRecipe = ({ item }: { item: Recipe }) => (
    ...
    >
      <View style={currentStyles.recipeHeader}>
        <Text style={currentStyles.recipeName} numberOfLines={1}>{item.name}</Text>
      ...
      <Text style={currentStyles.recipeSource} numberOfLines={1}>
      ...
      {item.tags.length > 0 && (
        <View style={currentStyles.tagsPreview}>
          ...
          {item.tags.length > 3 && (
            <Text style={currentStyles.moreTagsText}>+{item.tags.length - 3}</Text>
          )}
        </View>
      )}
    </TouchableOpacity>
    ...
    return (
      <ScrollView style={currentStyles.container}>
        <View style={currentStyles.content}>
          <Text style={currentStyles.editTitle}>Modifier la collection</Text>

          <Text style={currentStyles.label}>Nom de la collection *</Text>
          <TextInput
          ...
          <Text style={currentStyles.label}>Sélectionnez les tags *</Text>
          <View style={currentStyles.tagsContainer}>
            {availableTags.map(tag => {
              ...
              return (
                ...
                >
                  <Text style={[currentStyles.tagOptionText, isSelected && currentStyles.tagOptionTextSelected]}>
                    {tag}
                  </Text>
                </TouchableOpacity>
              );
    ...
    <View style={currentStyles.container}>
      <View style={currentStyles.header}>
        <View style={currentStyles.headerTop}>
          <Text style={currentStyles.title}>{collection.name}</Text>
      ...
      <View style={currentStyles.recipeCount}>
        <Text style={currentStyles.recipeCountText}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={currentStyles.emptyContainer}>
            ...
            <Text style={currentStyles.emptyText}>Aucune recette trouvée</Text>
      ...
      <TouchableOpacity style={currentStyles.deleteButton} onPress={handleDelete}>
        <Text style={currentStyles.deleteButtonText}>Supprimer cette collection</Text>
      </TouchableOpacity>
    </View>
  );
}
